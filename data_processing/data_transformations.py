...
def transform_statistics_fixtures(json_data):
    ...
    df["statistics"] = df["statistics"].apply(lambda x: {d['type']: d['value'] for d in x})
    df = df.groupby('fixture_id')['statistics'].apply(lambda x: pd.Series(x)).unstack().reset_index()
    # Rename columns
...
def transform_player_statistics(json_data):
    ...
    # Transform statistics to key:value pairs
    # TODO: make lambda more understandable
    df["players"] = df.apply(lambda row: [
        {
            "player_id": player["player"]["id"],
            "player_name": player["player"]["name"],
            **{stat: value for stats in player["statistics"] for stat, value in stats.items()}
        }
        for player in row["players"]
    ], axis=1)
    # Group home & away stats into array
    df = df.groupby('fixture_id')['players'].apply(lambda x: pd.Series(x)).unstack().reset_index()
    # Rename columns
...
def transform_events(json_data):
    ...
    # Get fixture_id from parameters
    df["fixture_id"] = json_data["parameters"]["fixture"]
    df = df.groupby("fixture_id").apply(lambda x: x.to_dict(orient='records')).reset_index(name='events')
    return df
